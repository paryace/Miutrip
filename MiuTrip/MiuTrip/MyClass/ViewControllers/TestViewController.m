...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        NSLog(@"tn=%@",tn);
        [UPPayPlugin startPay:tn sysProvide:@"12345678" spId:@"1234" mode:@"02" viewController:self delegate:self];
//        [UPPayPlugin startPay:tn mode:@"123456" viewController:self delegate:self];
    }
    [tn release];
    [connection release];
    connection = nil;
...
//    self.view.backgroundColor = [UIColor whiteColor];
    ...
    [btnStartPay setCornerRadius:5.0f];
    [btnStartPay setBorderColor:color(lightGrayColor) width:0.5];
    [btnStartPay setTitleColor:color(blueColor) forState:UIControlStateNormal];
    [btnStartPay setTitleColor:color(blackColor) forState:UIControlStateHighlighted];
    [self.view addSubview:btnStartPay];
    y += KBtn_height + KYOffSet;
    
    UIButton *returnBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [returnBtn setFrame:CGRectMake(btnStartPay.frame.origin.x, controlYLength(btnStartPay) + btnStartPay.frame.size.height, btnStartPay.frame.size.width, btnStartPay.frame.size.height)];
    ...
    [returnBtn setCornerRadius:5.0f];
    [returnBtn setBorderColor:color(lightGrayColor) width:0.5];
    [returnBtn setTitleColor:color(blueColor) forState:UIControlStateNormal];
